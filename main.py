...
def get_elenco(s):
    return [{'id':f'{s}-{i}','nome':f'{s} — {n}','posicoes':p} for i,(n,p) in enumerate(MODELO_ELENCO,1)]
...
def grant(code):
    ...
    get_db().execute('INSERT OR IGNORE INTO achievements(user_id,code) VALUES(?,?)',(session['user_id'],code)); get_db().commit()

def save_game(mode, score_for, score_against, champion=False):
    if 'user_id' not in session:return
    db=get_db(); db.execute('INSERT INTO games(user_id,mode,score_for,score_against,champion) VALUES(?,?,?,?,?)',(session['user_id'],mode,score_for,score_against,int(champion))); db.commit()
    grant('primeiro_jogo')
...
def montar_posicoes(f,e):
    r=[]; c={}
    for x in FORMACOES.get(f,FORMACOES['4-3-3']):
        c[x]=c.get(x,0)+1; k=f'{x}_{c[x]}'; r.append({'key':k,'codigo':x,'jogador':e.get(k)})
    return r
...
def preparar_jogadores(elenco,e,p,sel=None):
    ...
    for n,j in enumerate(elenco,1):
        op=[x for x in p if x['codigo'] in j['posicoes']]
        r.append({**j,'numero':n,'escolhido':j['id'] in usados,'selecionado':j['id']==sel,'tem_posicao_livre':any(not x['jogador'] for x in op)})
    return r
...
def register():
    if request.method=='POST':
        ...
        if not u or not p or not cp:flash('Preencha todos os campos.','error')
            ...
            try:
                db=get_db(); db.execute('INSERT INTO users(username,password_hash) VALUES(?,?)',(u,generate_password_hash(p))); db.commit(); flash('Conta criada.','success'); return redirect(url_for('login'))
...
def partida():
    ...
    if request.method=='POST':
        ...
        elif acao=='simular' and len(session.get('escolhidos',{}))>=11:
            base={'defensivo':(2,1),'equilibrado':(3,1),'ofensivo':(4,2)}.get(cfg['estilo'],(3,1));gf=random.randint(base[0],base[0]+4);ga=random.randint(0,3);save_game(cfg['modo'],gf,ga,gf>ga);session['resultado']={'gf':gf,'ga':ga,'champion':gf>ga};return redirect(url_for('resultado'))
...
@login_required
def daily():
    today=date.today().isoformat(); played=get_db().execute('SELECT id FROM games WHERE user_id=? AND mode=? AND date(created_at)=?',(session['user_id'],'daily',today)).fetchone()
    ...
    board=get_db().execute('SELECT u.username,MAX(g.score_for-g.score_against) saldo,MAX(g.score_for) gols FROM games g JOIN users u ON u.id=g.user_id WHERE g.mode="daily" AND date(g.created_at)=? GROUP BY g.user_id ORDER BY saldo DESC,gols DESC LIMIT 20',(today,)).fetchall()
...
def perfil():
    ...
    if request.method=='POST':
        ...
        if u and u!=session['username']:
            try:db.execute('UPDATE users SET username=? WHERE id=?',(u,session['user_id']));db.commit();session['username']=u
...
def multiplayer():
    ...
    if request.method=='POST':
        ...
        if action=='join':
            code=request.form.get('code','').upper();room=db.execute('SELECT * FROM rooms WHERE code=?',(code,)).fetchone()
            if room:
...
@login_required
def room(code):
    db=get_db();room=db.execute('SELECT * FROM rooms WHERE code=?',(code.upper(),)).fetchone()
    if not room:return redirect(url_for('multiplayer'))
    players=json.loads(room['players'])
    if request.method=='POST' and request.form.get('action')=='start' and room['owner_id']==session['user_id']:
        db.execute('UPDATE rooms SET started=1 WHERE code=?',(code.upper(),));db.commit();return redirect(url_for('room',code=code))
    return render_template('room.html',room=room,players=players,is_owner=room['owner_id']==session['user_id'])
