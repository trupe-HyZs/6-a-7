...
def close_db(_=None):
    ...
    if db:db.close()
def init_db():
    get_db().execute('CREATE TABLE IF NOT EXISTS users (id INTEGER PRIMARY KEY AUTOINCREMENT, username TEXT NOT NULL UNIQUE, password_hash TEXT NOT NULL, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)');get_db().commit()
...
def preparar_jogadores(elenco,escolhidos,posicoes,sel=None):
    usados=set(escolhidos.values());r=[]
    for numero,j in enumerate(elenco,1):
        op=[{'key':p['key'],'codigo':p['codigo'],'ocupada':bool(p['jogador'])} for p in posicoes if p['codigo'] in j['posicoes']]
        r.append({**j,'numero':numero,'escolhido':j['id'] in usados,'selecionado':j['id']==sel,'opcoes':op,'tem_posicao_livre':any(not x['ocupada'] for x in op)})
    return r
def sorteio_novo():
    atual=session.get('sorteio',{});ant=atual.get('selecao');op=[x for x in SELECOES if x!=ant] if ant else SELECOES
    return {'dado':random.randint(1,6),'selecao':random.choice(op),'copa':random.choice(COPAS)}
def trocar(tipo):
    atual=session['sorteio'];
    if tipo=='selecao': s=random.choice([x for x in SELECOES if x!=atual['selecao']]);c=atual['copa']
    else:s=atual['selecao'];c=random.choice([x for x in COPAS if x!=atual['copa']])
    return {'dado':random.randint(1,6),'selecao':s,'copa':c}
@app.route('/')
...
def register():
    if request.method=='POST':
        ...
        if not u or not p or not cp:flash('Preencha todos os campos.','error')
            ...
            try:
                db=get_db();db.execute('INSERT INTO users (username,password_hash) VALUES (?,?)',(u,generate_password_hash(p)));db.commit();flash('Conta criada. Agora faça login.','success');return redirect(url_for('login'))
...
def partida():
    ...
    if request.method=='POST':
        ...
        if acao=='sortear':
            if not session.get('sorteio') or session.get('rolagens_manuais',0)<3:
                session['sorteio']=sorteio_novo();session['rolagens_manuais']=session.get('rolagens_manuais',0)+(1 if session.get('sorteio') else 0);session['pode_rerolar']=False;session.pop('jogador_selecionado',None)
        elif acao in ('outra_selecao','outra_copa') and session.get('sorteio') and session.get('rolagens_manuais',0)<3:
            session['sorteio']=trocar('selecao' if acao=='outra_selecao' else 'copa');session['rolagens_manuais']+=1;session['pode_rerolar']=False;session.pop('jogador_selecionado',None)
